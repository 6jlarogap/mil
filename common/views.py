                    ...
                    if template == 'reports/report_2.html':
                        ...
                        if b.person_set.all().count() > 100:
                            if request.user.is_authenticated():
                                ...
                                return HttpResponseRedirect('%s?next=%s' % (reverse('login'), request.path + '%3F' + request.META['QUERY_STRING']))
                    ...
                    if template == 'reports/report_4.html' or template == 'reports/report_4a.html':
                        ...
                        def filter_data(burials_all, persons_all, region, district):
                            ...
                            if not burials_count and not known and not unknown:
                                ...
                                burials_data = {
                                    'all': burials_count,
                                    'WW': sum([c.count for c in conflicts if c.brief in [u"1МВ"]], 0),
                                    'WWII': sum([c.count for c in conflicts if c.brief in [u"2МВ"]], 0),
                                    'local': sum([c.count for c in conflicts if c.brief in [u"ДЛВК"]], 0),
                                    'other': sum([c.count for c in conflicts if c.brief not in [u"1МВ", u"2МВ", u"ДЛВК", ]], 0),
