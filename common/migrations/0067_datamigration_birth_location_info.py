...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Person = orm['common.Person']
        SimpleLocation = orm['common.SimpleLocation']
        
        print "Copying Person.birth_location.info to Person.birth_location_info"
        message_copied = " * %d of %d birth_location_info's copied"
        message_removed = " * %d empty birth_location's removed, %d: failed to remove"
        count_loc = count_removed_birth_locations = count_failed_to_remove = 0
        pp = Person.objects.filter(birth_location__info__isnull=False)
        count_loc_all = pp.count()
        for p in pp:
            if p.birth_location.info.strip():
                p.birth_location_info = p.birth_location.info
                # При импорте мы формируем birth_location, состоящую только из birth_location.info
                # После переноса p.birth_location.info в p.birth_location_info
                # p.birth_location отстанется пустой. Зачем эти пустышки в таблице SimpleLocation ?
                if not p.birth_location.country and not p.birth_location.obid:
                    loc = p.birth_location
                    p.birth_location = None
                    p.save()
                    try:
                        loc.delete()
                        count_removed_birth_locations += 1
                    except IntegrityError:
                        count_failed_to_remove += 1
                else:
                    p.save()
                count_loc += 1
                if count_loc % 500 == 0:
                    transaction.commit()
                    print message_copied % (count_loc, count_loc_all, )
                    print message_removed % (count_removed_birth_locations, count_failed_to_remove, )
        ...
        # После появления поля Person.birth_location_info поле SimpleLocation.info нигде не используется
        print "Removing info from SimpleLocation's"
        SimpleLocation.objects.update(info=None)
        transaction.commit()
        print " * done"
    ...
    models = {
        ...
        'common.georegion': {
            'Meta': {'ordering': "['name']", 'unique_together': "(('country', 'name'),)", 'object_name': 'GeoRegion'},
            'country': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['common.GeoCountry']", 'null': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '36', 'db_index': 'True'}),
            'obid': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'})
        ...
        'common.municipalitet': {
            'Meta': {'ordering': "['name']", 'unique_together': "(('district', 'name'),)", 'object_name': 'Municipalitet'},
        ...
        'common.person': {
            ...
            'birth_location': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['common.SimpleLocation']", 'null': 'True', 'blank': 'True'}),
            'birth_location_info': ('django.db.models.fields.TextField', [], {'null': 'True', 'blank': 'True'}),
        ...
        'common.personduty': {
            ...
            'unit_name': ('django.db.models.fields.CharField', [], {'db_index': 'True', 'max_length': '255', 'null': 'True', 'blank': 'True'})
        },
