...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding index on 'MilitaryConflict', fields ['name']
        db.create_index('common_militaryconflict', ['name'])

        # Adding index on 'DeathCause', fields ['name']
        db.create_index('common_deathcause', ['name'])

        # Adding index on 'BurialType', fields ['name']
        db.create_index('common_burialtype', ['name'])

        # Adding index on 'Nationality', fields ['name']
        db.create_index('common_nationality', ['name'])

        # Adding index on 'Rank', fields ['name']
        db.create_index('common_rank', ['name'])

        # Adding index on 'DeadmanCategory', fields ['name']
        db.create_index('common_deadmancategory', ['name'])

        # Adding index on 'MilitaryConflictType', fields ['full']
        db.create_index('common_militaryconflicttype', ['full'])

        # Adding index on 'MilitaryUnit', fields ['name']
        db.create_index('common_militaryunit', ['name'])

        # Adding index on 'Comissariat', fields ['name']
        db.create_index('common_comissariat', ['name'])

        # Adding index on 'MemorialState', fields ['name']
        db.create_index('common_memorialstate', ['name'])

        # Adding index on 'Post', fields ['name']
        db.create_index('common_post', ['name'])


    def backwards(self, orm):
        
        # Removing index on 'Post', fields ['name']
        db.delete_index('common_post', ['name'])

        # Removing index on 'MemorialState', fields ['name']
        db.delete_index('common_memorialstate', ['name'])

        # Removing index on 'Comissariat', fields ['name']
        db.delete_index('common_comissariat', ['name'])

        # Removing index on 'MilitaryUnit', fields ['name']
        db.delete_index('common_militaryunit', ['name'])

        # Removing index on 'MilitaryConflictType', fields ['full']
        db.delete_index('common_militaryconflicttype', ['full'])

        # Removing index on 'DeadmanCategory', fields ['name']
        db.delete_index('common_deadmancategory', ['name'])

        # Removing index on 'Rank', fields ['name']
        db.delete_index('common_rank', ['name'])

        # Removing index on 'Nationality', fields ['name']
        db.delete_index('common_nationality', ['name'])

        # Removing index on 'BurialType', fields ['name']
        db.delete_index('common_burialtype', ['name'])

        # Removing index on 'DeathCause', fields ['name']
        db.delete_index('common_deathcause', ['name'])

        # Removing index on 'MilitaryConflict', fields ['name']
        db.delete_index('common_militaryconflict', ['name'])
    ...
    models = {
        ...
        'common.person': {
            ...
            'obid': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'}),
            'oblocationid': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'}),
            'outside_rb': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
