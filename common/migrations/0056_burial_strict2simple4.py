...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        for b in orm.Burial.objects.all():
            keys = [f.name for f in b.old_location._meta.fields]
            keys.remove('id')
            data = dict([(k, getattr(b.old_location, k)) for k in keys])

            b.location = orm.SimpleLocation.objects.create(**data)
            b.save()
